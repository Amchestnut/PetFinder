...
def build_custom_multimodal_model(tabular_dim):
    ...
    x = image_input
    # Image Augmentation
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.RandomFlip("horizontal"))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.RandomRotation(0.1))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.RandomZoom(0.1))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.RandomContrast(0.1))(x)
...
if __name__ == "__main__":
    ...
    # 7. Save
    model.save(PROJECT_ROOT / "models" / "model_v5_multimodal.keras")
