...
def create_dataset(X_tab, X_img_paths, y, is_train=True):
    """
    This is different in this model
    Now returns image_input of shape (K=3, H, W, 3)
    """
...
def build_custom_multimodal_model(tabular_dim):
    """
    I did this, because TimeDistributed applies the SAME CNN to each of the 3 images.
    Input: image_input (None, 3, 128, 128, 3)
    """
    image_input = tf.keras.Input(shape=(K_IMAGES, IMG_SIZE[0], IMG_SIZE[1], 3), name="image_input")
    tabular_input = tf.keras.Input(shape=(tabular_dim,), name="tabular_input")

    # PART 1: Custom CNN
    x = image_input
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Rescaling(1./255))(x)   # rescaling
    ...
    # Aggregate across 3 images
    img_feats = tf.keras.layers.GlobalAveragePooling1D()(x)
    ...
    # PART 3: Merge
    combined = tf.keras.layers.concatenate([t, img_feats])
...
if __name__ == "__main__":
    ...
    # 7. Save
    model.save(PROJECT_ROOT / "models" / "model_v3_multimodal.keras")
